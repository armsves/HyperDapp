    ...
    <div className="py-8">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
        {sorted.map((dapp) => {
          const dappId = (dapp as unknown as { id: string }).id;
          return (
          <div key={dappId} className="border rounded-xl p-5 bg-white shadow-sm">
            <div className="flex items-start justify-between">
              <div>
                <h3 className="font-semibold text-lg">{dapp.name}</h3>
                <p className="text-xs text-muted-foreground font-mono mt-1">{dappId}</p>
            ...
            >
              {savingId === dappId ? 'Saving…' : dapp.active ? 'Deactivate' : 'Activate'}
            </button>
          </div>
          );
        })}
